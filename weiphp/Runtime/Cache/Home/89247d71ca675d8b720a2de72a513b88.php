          ...
          <table cellspacing="1">
            ...
            <tbody>
              <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                  ...
                  <td>
                      <a href="<?php echo U('edit',array('id'=>$data['id'],'model'=>$model['id']));?>">编辑</a>
                      <a href="<?php echo U('del',array('id'=>$data['id'],'model'=>$model['id']));?>">删除</a>
                      <a class="shop_qrcode" href="<?php echo U('sence_qr',array('id'=>$data['id'],'model'=>$model['id']));?>">门店二维码</a>
                  </td>
